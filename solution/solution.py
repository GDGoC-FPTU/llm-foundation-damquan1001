...
def compare_models(prompt: str) -> dict:
    ...
    def calc_cost(model_name: str, usage: dict) -> float:
        prices = PRICING_1M_TOKENS[model_name]
        return (usage["input_tokens"] * prices["input"] + usage["output_tokens"] * prices["output"]) / 1_000_000.0

    gpt4o_cost = calc_cost("gpt-4o", gpt4o_usage)
    mini_cost = calc_cost("gpt-4o-mini", mini_usage)
    gemini_cost = calc_cost("gemini-2.5-flash", gemini_usage)
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        # Limit history to the last 3 turns (max 6 messages)
        history = history[-6:]
    ...
    for item in results:
        ...
        for key in ["gpt4o", "gpt4o_mini", "gemini_flash"]:
            if key in item:
                ...
                truncated_resp = response.replace("\n", " ")
                if len(truncated_resp) > 50:
                    truncated_resp = truncated_resp[:47] + "..."
                
                latency_str = f"{stats['latency']:.2f}s"
                tokens_str = f"{stats['input_tokens']}/{stats['output_tokens']}"
                cost_str = f"${stats['cost']:.6f}"
                
                lines.append(f"| {prompt} | {model_name} | {truncated_resp} | {latency_str} | {tokens_str} | {cost_str} |")
